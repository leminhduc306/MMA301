    ...
    <View style={{ flex: 1 }}>
      <Tab.Navigator
        ...
        tabBar={(props) => {
          ...
          console.log('=== TabBar routes ===', props.state.routes.map(r => r.name));

          return (
            <View>
              {/* MiniPlayer - Chỉ hiển thị cho USER, ẩn cho ADMIN */}
              {userRole !== 'ADMIN' && <MiniPlayer />}
              <View style={{ flexDirection: 'row', backgroundColor: '#1a1a1a', borderTopColor: '#262626', borderTopWidth: 1, paddingBottom: 8, paddingTop: 8, height: 60 }}>
                {props.state.routes.map((route, index) => {
                  ...
                  return (
                    ...
                    >
                      {IconComponent && IconComponent({ color: isFocused ? '#1DB954' : '#888', size: 24 })}
                      <Text style={{ color: isFocused ? '#1DB954' : '#888', fontSize: 10, marginTop: 4 }}>
                        {label}
                      </Text>
                    </TouchableOpacity>
                  );
